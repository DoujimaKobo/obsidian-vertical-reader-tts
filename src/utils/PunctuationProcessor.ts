...
export class PunctuationProcessor {
  /**
   * 「間」を表す記号を読点（、）に正規化して、TTSに休止させる。
   *
   * … ‥ ― — などは多くのTTSでそのまま読み飛ばされ休止が入らないため、
   * 読点に1:1で置換する。文字数を変えないのでハイライトのオフセットもずれない。
   * 長音記号（ー / U+30FC）は母音記号なので対象外。
   *
   * 例: 「そうか……」→「そうか、、」 / 「――だが」→「、、だが」
   */
  static normalizePauseSymbols(text: string): string {
    // …(U+2026) ‥(U+2025) ―(U+2015) —(U+2014)
